...
export default function AllApprovedComplaints() {
  ...
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [upvotingId, setUpvotingId] = useState(null); // For loading state
  ...
  useEffect(() => {
    const fetchComplaints = async () => {
      try {
        ...
        let allComplaints = res.data?.data?.results || res.data?.results || res.data?.data || [];
  ...
  const handleToggleUpvote = async (reference_id, index) => {
    if (upvotingId) return; // Prevent multiple clicks
    ...
    try {
      ...
      setComplaints(prev => {
        ...
        updated[index] = {
          ...updated[index],
          upvotes_count: res.data.upvotes_count || updated[index].upvotes_count,
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {complaints.map((complaint, index) => (
            ...
            >
              ...
              >
                <ThumbsUp size={18} className={complaint.has_upvoted ? "fill-current" : ""} />
