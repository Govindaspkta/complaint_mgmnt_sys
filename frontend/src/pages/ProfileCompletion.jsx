...
export default function ProfileCompletion() {
  ...
  const fetchProfile = async () => {
    try {
      ...
      // IMPORTANT FIX:
      // Only treat this as a real, existing profile if it actually has
      // a verification_status. An empty object ({}) or a response with
      // no meaningful fields means the user hasn't submitted yet.
      ...
      if (hasRealProfile) {
        setProfile(raw);
        setFormData({
          citizenship_number: raw.citizenship_number || "",
          address: raw.address || "",
          dob: raw.dob || "",
        });

        if (raw.profile_picture) setProfilePreview(raw.profile_picture);
        if (raw.citizenship_front) setFrontPreview(raw.citizenship_front);
        if (raw.citizenship_back) setBackPreview(raw.citizenship_back);
      } else {
        setProfile(null);
      }
    } catch (err) {
  ...
  const getStatus = () => {
    ...
    const st = profile.verification_status;
    if (st === "PENDING") return { color: "bg-yellow-100", text: "PENDING - Under Review" };
    if (st === "APPROVED") return { color: "bg-green-100", text: "APPROVED - Cannot Modify" };
    if (st === "REJECTED") return { color: "bg-red-100", text: "REJECTED - Resubmit Allowed" };
    return { color: "bg-gray-100", text: st };
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccess("Submitted! Now PENDING.");
      fetchProfile();
    } catch (err) {
      setErrors({ api: "Failed" });
    } finally {
      setLoading(false);
    }
  };

  if (pageLoading) return <p className="text-center py-20 text-lg">Loading profile...</p>;
    ...
    <div className="min-h-screen bg-slate-50 py-10 px-4">
      <div className="max-w-5xl mx-auto bg-white rounded-3xl shadow-xl overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-8">
          ...
          <div className="flex justify-center">
            <div className="text-center">
              <img src={profilePreview || "https://ui-avatars.com/api/?name=User&background=e5e7eb&color=6b7280&size=200"} alt="Profile" className="w-36 h-36 rounded-full object-cover border-4 border-slate-200 shadow" />
          ...
          <div>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                <label className="block mb-3 font-medium">Citizenship Front</label>
                <label className={`cursor-pointer block border-2 border-dashed border-slate-300 rounded-2xl overflow-hidden ${!canSubmit() ? 'opacity-50' : ''}`}>
                  {frontPreview ? <img src={frontPreview} alt="Front" className="h-60 w-full object-cover" /> : <div className="h-60 flex items-center justify-center text-slate-400">Click to upload front image</div>}
              ...
              <div>
                <label className="block mb-3 font-medium">Citizenship Back</label>
                <label className={`cursor-pointer block border-2 border-dashed border-slate-300 rounded-2xl overflow-hidden ${!canSubmit() ? 'opacity-50' : ''}`}>
                  {backPreview ? <img src={backPreview} alt="Back" className="h-60 w-full object-cover" /> : <div className="h-60 flex items-center justify-center text-slate-400">Click to upload back image</div>}
          ...
          <button type="submit" disabled={loading || !canSubmit()} className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-slate-400 text-white py-4 rounded-xl font-semibold transition">
            {loading ? "Submitting..." : canSubmit() ? "Submit Profile" : "Cannot Modify Now"}
          </button>
        </form>
      </div>
    </div>
