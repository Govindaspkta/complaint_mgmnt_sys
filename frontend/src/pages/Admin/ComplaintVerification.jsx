...
//     } catch (err) {
//       console.error(err);
//       alert(err.response?.data?.message || err.response?.data?.detail || "Failed to approve complaint");
//     } finally {
//       setProcessingId(null);
//     }
//   };
...
//     } catch (err) {
//       console.error(err);
//       alert(err.response?.data?.message || err.response?.data?.detail || "Failed to reject complaint");
//     } finally {
//       setProcessingId(null);
//     }
//   };
...
//             <ShieldCheck className="text-green-600" /> Complaint Verification
//           </h1>
//           <p className="text-gray-600">Review and take action on pending complaints</p>
//         </div>
//         <button 
//           onClick={fetchComplaints} 
//           className="bg-gray-100 hover:bg-gray-200 px-5 py-2.5 rounded-xl text-sm font-medium transition"
//         >
//           ↻ Refresh
//         </button>
//       </div>
...
//               <div className="flex justify-between items-start mb-5">
//                 <div>
//                   <h2 className="text-xl font-bold line-clamp-2">{complaint.title}</h2>
//                   <p className="text-gray-500 text-sm mt-1">
//                     {complaint.category?.display_name || complaint.category} • {complaint.priority}
//                   </p>
//                 </div>
//                 <span className="px-4 py-1 bg-yellow-100 text-yellow-700 rounded-full text-xs font-medium">PENDING</span>
//               </div>
...
//               <div className="text-sm space-y-2 mb-6 bg-gray-50 p-4 rounded-2xl">
//                 <p><strong>Location:</strong> {complaint.municipality}, Ward {complaint.ward}</p>
//                 <p><strong>Submitted by:</strong> {complaint.user?.username || "Anonymous"}</p>
//               </div>
...
//                   src={complaint.image} 
//                   alt="Evidence" 
//                   className="w-full h-52 object-cover rounded-2xl mb-6 border" 
//                 />
//               )}
...
//               value={rejectReason}
//               onChange={(e) => setRejectReason(e.target.value)}
//               placeholder="Write detailed reason for rejection..."
//               className="w-full border border-gray-300 p-4 rounded-2xl h-40 focus:outline-none focus:border-red-500"
//             />

//             <div className="flex gap-4 mt-6">
//               <button 
//                 onClick={() => { setRejectModal(null); setRejectReason(""); }}
//                 className="flex-1 py-4 border border-gray-300 rounded-2xl hover:bg-gray-100"
//               >
//                 Cancel
//               </button>
//               <button 
//                 onClick={handleReject} 
//                 className="flex-1 bg-red-600 text-white py-4 rounded-2xl hover:bg-red-700"
//               >
//                 Confirm Reject
//               </button>
//             </div>
...
//           <p className="text-gray-600">Review and take action on pending complaints</p>
//         </div>
//         <button onClick={fetchComplaints} className="bg-gray-100 hover:bg-gray-200 px-5 py-2.5 rounded-xl text-sm font-medium transition">
//           ↻ Refresh
//         </button>
//       </div>
...
//               {complaint.image && (
//                 <img src={complaint.image} alt="Evidence" className="w-full h-52 object-cover rounded-2xl mb-6 border" />
//               )}
...
import { useEffect, useState, useCallback } from "react";
import { getAdminComplaints, updateComplaintAdmin } from "../../api/complaintApi";
import { ShieldCheck } from 'lucide-react';
...
export default function ComplaintVerification() {
  ...
  const fetchComplaints = useCallback(async () => {
    try {
      ...
      const res = await getAdminComplaints();   // ← Using Admin API

      let data = res.data?.data?.results || res.data?.results || res.data?.data || res.data || [];

      // Show both PENDING and REJECTED
      const filtered = data.filter(c => {
        const status = (c.status || c.complaint_status || "").toString().toUpperCase();
  ...
  if (loading) return <div className="text-center py-20 text-lg">Loading complaints...</div>;
  if (error) return <div className="text-red-600 text-center py-10">{error}</div>;
    ...
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600">Review Pending and Rejected complaints</p>
        </div>
        <button onClick={fetchComplaints} className="bg-gray-100 hover:bg-gray-200 px-5 py-2.5 rounded-xl text-sm font-medium">
          ↻ Refresh
        </button>
      </div>
      ...
      {complaints.length === 0 ? (
        <div className="bg-white rounded-3xl p-16 text-center shadow">
          <p className="text-xl text-gray-500">No pending or rejected complaints.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {complaints.map((complaint) => {
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
          <div className="bg-white p-8 rounded-3xl w-full max-w-md">
            ...
            <textarea
              ...
              onChange={(e) => setRejectReason(e.target.value)}
              placeholder="Write detailed reason for rejection..."
              className="w-full border border-gray-300 p-4 rounded-2xl h-40"
            />
            <div className="flex gap-4 mt-6">
              <button
                onClick={() => { setRejectModal(null); setRejectReason(""); }}
