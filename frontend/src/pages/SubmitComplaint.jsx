...
const getProfile = () => api.get("/authx/profile-completion/details/");
...
export default function SubmitComplaint() {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        const status = (profile.verification_status || "").toString().trim().toUpperCase();

        console.log("✅ Verification Status (normalized):", status);
  ...
  const isProfileVerified = profileStatus === "APPROVED";
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isProfileVerified) {
      alert("❌ Your profile must be APPROVED before submitting complaints.");
      return;
    }
  ...
  if (isProfileLoading) {
    return (
      <div className="max-w-4xl mx-auto px-6 py-20 text-center">
        <p className="text-xl">Checking your profile verification status...</p>
      </div>
    );
  }
    ...
    return (
      <div className="max-w-4xl mx-auto px-6 py-10">
        <div className="bg-white rounded-3xl shadow-xl p-12 text-center">
          <h1 className="text-4xl font-bold text-gray-800 mb-4">Profile Verification Required</h1>
          <p className="text-red-600 text-2xl mb-6">
            Current Status: <strong>{profileStatus || "Not Submitted"}</strong>
          </p>
          <p className="text-gray-600 mb-10 max-w-md mx-auto">
            Only users with <strong>APPROVED</strong> profile can submit complaints.
          </p>
          <button
            onClick={() => window.location.href = "/profile-completion"}
            className="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-10 py-4 rounded-2xl text-lg transition"
          >
            Complete / Verify Profile Now
          </button>
        </div>
      </div>
    );
  ...
  // Main Form - Shown only when APPROVED
  return (
    <div className="max-w-4xl mx-auto px-6 py-10">
