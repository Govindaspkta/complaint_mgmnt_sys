      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex justify-between items-center h-16">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold">Hamro Aawaj</h1>
          ...
          <div className="flex items-center gap-8 text-sm font-medium">
            <Link
              to={user ? "/all-approved-complaints" : "/"}
              className="flex items-center gap-2 hover:text-accent"
            >
              <Home size={20} />
              {user ? "Approved Complaints" : "Home"}
            </Link>

            <Link to="/complaints" className="flex items-center gap-2 hover:text-accent">
              <Users size={20} />
              My Complaints
            </Link>

            <Link to="/submit-complaint" className="flex items-center gap-2 hover:text-accent">
              <PlusCircle size={20} />
              Lodge Complaint
            </Link>
            ...
            {!user ? (
              ...
              <div className="relative" ref={dropdownRef}>
                <div
                  onClick={() => setOpen(!open)}
                  className="w-10 h-10 bg-primary-500 rounded-full flex items-center justify-center cursor-pointer font-bold hover:ring-2 hover:ring-offset-2 hover:ring-primary-400 transition"
                >
                  {user?.username?.charAt(0).toUpperCase() || "U"}
                </div>

                {open && (
                  <div className="absolute right-0 mt-3 w-56 bg-white text-black rounded-xl shadow-lg py-2 z-50">
                    <Link to="/profile-completion" className="block px-4 py-3 border-b hover:bg-gray-100">
                      <p className="font-semibold">{user.username}</p>
                      <p className="text-xs text-gray-500">Complete Profile</p>
                    </Link>
